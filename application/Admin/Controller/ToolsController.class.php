...
//            }
    ...
    public function upload_picture()
    {
        $config = [
            'maxSize'	=> 3145728,
            'rootPath'	=> './',
            'savePath'	=> '/Uploads/image/tailor/'.$_REQUEST[''],
            'saveName'	=> ['uniqid',''],
            'exts'		=> ['jpg', 'gif', 'png', 'jpeg'],
            'autoSub'	=> true,
            'subName'	=> ['date','Ymd'],
        ...
        if ($info){
            ...
            echo json_encode(array('status' => 'ok', 'info' => $url));
        } else {
            echo json_encode(array('status' => 'error', 'info' => $uploader->getError()));
        }
    }
    ...
    public function upload_comment_picture()
    {
        ...
        if ($info){
            ...
            $img['img'] = $url;
            $img['thumb'] = $arr;
            echo json_encode(array('status' => 'ok', 'info' => $img));
        } else {
            echo json_encode(array('status' => 'error', 'info' => $uploader->getError()));
        }
    }
    ...
    public function upload_video(){
        ...
        $info = $uploader->upload();
        if ($info){
            $url = '/Uploads/video/'.$info['video']['savename'];
    ...
    public function upload_music(){
        $config = [
            'maxSize'	=> 0,
            'rootPath'	=> './Uploads/music/',
            'savePath'	=> '',
            'saveName'	=> ['uniqid',''],
            'exts'		=> ['mp3','wav','wma','ogg','ape','acc'],
            'autoSub'	=> true,
            'subName'	=> '',
        ];
        $uploader = new Upload($config);
        $info = $uploader->upload();
        if ($info){
            $url = '/Uploads/music/'.$info['video']['savename'];
            echo json_encode(array('status' => 'ok', 'url' => $url));
        }else {
            echo json_encode(array('status'=>'error','info'=>$uploader->getError()));
            die;

...
<div class="ml170" id="page-content" style="overflow:auto;min-height:100%">
	<?php echo W('public/breadcrumbs',[ [ ['href'=>U('Auth/index'),'text'=>'权限管理'], ['href'=>'','text'=>'规则列表'] ] ]);?>
...
<!--    <div class="text-c">
    ...
    <div class="cl pd-5 bg-1 bk-gray">
        <span class="l">
            ...
            <a class="btn btn-primary radius"  href="<?php echo U('Auth/editRule');?>">
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <tr class="text-c">
                ...
                <th>节点类型</th>
                <th>操作 /
                    <a class=""  href="<?php echo U('Auth/editRule');?>" thtle="添加节点">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td style="text-align: left">
                    <?php if($v['is_button'] == 2): ?><div><?php echo ($v["title"]); ?></div>
                        <?php elseif($v['is_button'] == 1): ?>
                        <div>|——————<?php echo ($v["title"]); ?></div>
                        <?php else: ?>
                        <div>|——————<?php echo ($v["title"]); ?></div><?php endif; ?>
                </td>
                <td style="text-align: left"><?php echo ($v["name"]); ?></td>
                <td><?php echo ($v['status']?'开启':'关闭'); ?></td>
                <td>
                    <?php if($v['is_button'] == '0'): ?>视图
                        <?php elseif($v['is_button'] == '1'): ?>操作
                        <?php elseif($v['is_button'] == '2'): ?>模块<?php endif; ?>
                </td>
                <td class="td-manage">
                    <a style="text-decoration:none" class="ml-5"  href="<?php echo U('Auth/editRule',['id'=>$v['id']]);?>" title="编辑">
                        <i class="Hui-iconfont">&#xe6df;</i>
                    </a>
                    <a style="text-decoration:none" class="ml-5" onClick="picture_del(this,<?php echo ($v["id"]); ?>)" href="javascript:;" title="删除">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                    </a>
                </td>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        $.post("<?php echo U('Auth/delRule');?>", {ids:kid}, function(data){
            if( data.status == 'ok' ){
    ...
    function picture_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Auth/delRule');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){

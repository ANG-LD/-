...
<div class="ml170" id="page-content" style="overflow:auto;min-height:100%">
	<?php echo W('public/breadcrumbs',[ [ ['href'=>U('Live/record'),'text'=>'名师录播'], ] ]);?>
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    ...
                    <td class="td-manage" style="text-align: center">
                        <a title="播放" href="javascript:;" onclick="sel('<?php echo U('Live/play_record',['id'=>$l['live_store_id']]);?>')"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e6;</i></a>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Live/del_live_store');?>", {ids:kid}, function(data){
                if( data.status == 'ok' ){
    ...
    function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Live/del_live_store');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
